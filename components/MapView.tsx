...
const MapView: React.FC<MapViewProps> = ({ filteredUsers }) => {
  ...
  // Keep track of existing markers to update them instead of efficiently
  const userMarkersRef = useRef<{ [key: string]: any }>({});
  const spotMarkersRef = useRef<{ [key: string]: any }>({});
  ...
  useEffect(() => {
    ...
    // --- USERS SYNC ---
    const activeUserIds = new Set<string | number>();
    ...
    displayUsers.forEach(user => {
      ...
      // HTML for the icon
      const iconHtml = `<div class="relative animate-scale-in">
                 ...
                 <div class="absolute -bottom-2 left-1/2 -translate-x-1/2 bg-black/50 text-white text-[8px] px-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">${user.name}</div>
               </div>`;

      const existingMarker = userMarkersRef.current[user.id];

      if (existingMarker) {
        // Update position smoothly
        existingMarker.setLatLng([user.location.lat, user.location.lng]);
        // Update icon if needed (e.g. if 'isMe' status changed, though unlikely for same ID)
        // For performance, we might skip icon update if not needed, but here we enforce it to be safe
        const newIcon = L.divIcon({ html: iconHtml, className: 'group', iconSize: [48, 48], iconAnchor: [24, 24] });
        existingMarker.setIcon(newIcon);

        // Z-Index: Ensure proper stacking. "Me" always on top.
        existingMarker.setZIndexOffset(isMe ? 1000 : 0);
      } else {
        // Create new marker
        const icon = L.divIcon({ html: iconHtml, className: 'group', iconSize: [48, 48], iconAnchor: [24, 24] });
        const marker = L.marker([user.location.lat, user.location.lng], { icon, zIndexOffset: isMe ? 1000 : 0 }).addTo(userLayer);
    ...
    Object.keys(userMarkersRef.current).forEach(id => {
      // Convert to number if possible for comparison, or string to string
      // Our IDs are numbers mostly.
      const idNum = parseInt(id);
      const key = isNaN(idNum) ? id : idNum;

      if (!activeUserIds.has(key)) {
        userLayer.removeLayer(userMarkersRef.current[id]);
        delete userMarkersRef.current[id];
      }
    });
  }, [displayUsers, currentUser, viewProfile]); // Split effects to avoid re-running everything
  ...
  useEffect(() => {
    ...
    // --- SPOTS SYNC ---
    const activeSpotIds = new Set<number>();

    filteredSpots.forEach(spot => {
      activeSpotIds.add(spot.id);
      ...
      const existingMarker = spotMarkersRef.current[spot.id];

      if (existingMarker) {
        existingMarker.setLatLng([spot.location.lat, spot.location.lng]);
        const newIcon = L.divIcon({ html: iconHtml, className: '', iconSize: [40, 40], iconAnchor: [20, 20] });
        existingMarker.setIcon(newIcon);
        existingMarker.setZIndexOffset(isSelected ? 500 : 100);
      } else {
        const icon = L.divIcon({ html: iconHtml, className: '', iconSize: [40, 40], iconAnchor: [20, 20] });
        const marker = L.marker([spot.location.lat, spot.location.lng], { icon, zIndexOffset: isSelected ? 500 : 100 }).addTo(spotLayer);
        marker.on('click', () => {
    ...
    // Cleanup Removed Spots
    Object.keys(spotMarkersRef.current).forEach(idStr => {
      const id = parseInt(idStr);
      if (!activeSpotIds.has(id)) {
        spotLayer.removeLayer(spotMarkersRef.current[id]);
  ...
  const handleLiveLocate = async () => {
    ...
    try {
      ...
      if (mapInstanceRef.current) {
        mapInstanceRef.current.flyTo([newCoords.lat, newCoords.lng], 16, { animate: true, duration: 1.5 });
        // Force update the currentUser marker immediately to ensure visual sync using ref
        if (userMarkersRef.current[currentUser.id]) {
          userMarkersRef.current[currentUser.id].setLatLng([newCoords.lat, newCoords.lng]);
        }
      }
    } catch (err) {
