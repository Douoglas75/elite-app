...
const BottomNavItem: React.FC<{ tab: ActiveTab; icon: IconName; label: string; }> = ({ tab, icon, label }) => {
    ...
    return (
      <button
        onClick={() => selectTab(tab)}
        className="flex flex-col items-center justify-center gap-1 w-full h-full relative outline-none touch-manipulation"
      >
...
const Layout: React.FC<LayoutProps> = ({ children }) => {
    const { viewingUser, activeChatThreadId, activeSubView } = useAppContext();
    ...
    return (
        <div className="h-full w-full bg-[#050B14] flex flex-col md:flex-row overflow-hidden select-none">
            ...
            <div className="flex-1 flex flex-col relative overflow-hidden h-full z-0">
                ...
                {/* Mobile Bottom Nav - Fixed for better clickability */}
                {showNavBar && (
